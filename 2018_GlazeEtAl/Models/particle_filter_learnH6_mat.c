...
void particle_filter_learnH6_mat(
        double K,
        double *H0pr,
        int H0len,
        double *xpr,
...
{
    double F, K, *H0pr, *xpr, *musgnpr, *Lexp_pr, partM, *etapr, etalen, *Lvar_pr, maxL, *Hmat_pr;
    int N, Hlen, simN;
    ...
    plhs[2] = mxCreateDoubleMatrix(partM, N, mxREAL);
    
    Lexp_pr = mxGetPr(plhs[0]);
    Lvar_pr = mxGetPr(plhs[1]);
    Hmat_pr = mxGetPr(plhs[2]);
    
    /* Do the actual computations in a subroutine */
    particle_filter_learnH6_mat(K,H0pr,Hlen,xpr,musgnpr,Lexp_pr,N,partM,simN,F,etapr,etalen,Lvar_pr,Hmat_pr);
