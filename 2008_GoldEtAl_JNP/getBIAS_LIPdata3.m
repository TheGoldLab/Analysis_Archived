function [bs_,rocs_,ds_,fs_]   = getBIAS_LIPdata3(sid, sldat)
% function [bs_,rocs_,ds_,fs_] = getBIAS_LIPdata3(sid, sldat)
...
% linear fit of slopes
Lg    = Lgood & isfinite(sldat) & isfinite(chcs) & isfinite(cohs) & isfinite(wk);
A     = [double(Lg&Ltin) double(Lg&~Ltin) cohs wk];
A(~Ltin,3:4) = -A(~Ltin, 3:4);
[f,e] = lscov(A(Lg,:), sldat(Lg));
fs_   = [f' e'];
