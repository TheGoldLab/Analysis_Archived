function [output]= fit_auditoryModel_spatialAudWidth(input)
...
if ~isfield(input, 'computeNLLflag')|~input.computeNLLflag
    ...
    options=optimset(oldOpts, 'maxFunEvals', 100000000, 'MaxIter', 10e6);
...
% inputs they are (wp)
    function [negLLik] = audModel(params)                %input comes from fmincon (initially start_point)
        ...
        [negLLik, output] = auditoryModel_gen_spatialAudWidth(input);
