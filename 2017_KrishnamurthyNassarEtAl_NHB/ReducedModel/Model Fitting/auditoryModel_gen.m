function [negLLik, input] = auditoryModel_gen(input)
...
for i = 1:max(input.blockNum)
    ...
    if input.genDataFlag
        ...
        for ind=1:length(allK);
            ...
            internalLike = normrnd(input.outcome(selSound), audLikeWid);
            end
          
            % generate posterior perception:
            gridTicks = 0:180;
            [postDist,~,~] = ...
                combinePriorAndAllLikes(input.prediction(k), blkModelPredSigma(k),...
                internalLike, audLikeWid, hazard);
...
for k = 1:length(input.prediction)
    ...
    if input.subjectiveFlag==true;  %we use the subject prediction instead of the model mean
        ...
        blkModelPredSigma(k), input.outcome(selSound), audLikeWid, hazard);
    else
     %   keyboard
