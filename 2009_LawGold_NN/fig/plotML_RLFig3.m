% plotML_RLfig3.m
% plot changes in weights with training
...
DIRS      = [180 0];
SIMNUM    = [81:90];
[wavg_c, wse_c] = getML_RLFig3AvgWSortedByTh(Monk, NSEN, NTUNE, ELN, DIRS, SIMNUM, TRIALS, THBINS, recompute);


% optimal weights
NORMSCALE = sqrt(0.02);
[woavg_c, wod, woc_c, wocd] = getML_RLFig3OptimalWSortedByTh(Monk, NSEN, NTUNE, ELN, DIRS, SIMNUM, THBINS, NORMSCALE, recompute);
...
% % plot annotation arrow 
% [xb,yb] = axisXY2figXY(0,1.2*clim_(1));
% [xe,ye] = axisXY2figXY(0,1.3*clim_(1));
% anh     = annotation('arrow', [xb xe], [yb ye]);
% set(anh, 'Color', 'r', 'LineWidth', 0, 'LineStyle', '-', 'HeadStyle', 'cback2', ...
%           'HeadWidth', 1.5*ms, 'HeadLength', 1.5*ms)
% [xb,yb] = axisXY2figXY(180,1.2*clim_(1));
% [xe,ye] = axisXY2figXY(180,1.3*clim_(1));
% anh     = annotation('arrow', [xb xe], [yb ye]);
% set(anh, 'Color', 'b', 'LineWidth', 0, 'LineStyle', '-', 'HeadStyle', 'cback2', ...
%           'HeadWidth', 1.5*ms, 'HeadLength', 1.5*ms)
