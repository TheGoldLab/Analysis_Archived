function [th thd] = getML_RLSimulatedThForMT(Monk, N, recompute)
% simulate discrimination thresholds for model MT neurons 

[h] = dirnames;
fn  = ['getML_RLSimulatedThForMT_' Monk '_' int2str(N) '.mat'];
    ...
    for i = 1:length(PREF)
        if ~isnan(PREF(2,i)) & ~isnan(NULL(2,i)) & ~isnan(FANO(i)) & PREF(2,i)>0.5
            ...
            for j = 1:length(COHS)
                ...
                m  = bl+COHS(j)*PREF(2,i);
                sd = sqrt(FANO(i)*abs(bl+COHS(j)*PREF(2,i)));
                rp = m+sd*randn(N,1);

                m  = bl+COHS(j)*NULL(2,i);
                sd = sqrt(FANO(i)*abs(bl+COHS(j)*NULL(2,i)));
                rn = m+sd*randn(N,1);
