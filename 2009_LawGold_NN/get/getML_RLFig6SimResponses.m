function [coeff, score, latent, rall] = getML_RLFig6SimResponses(Monk, NSEN, NTUNE, ELN, SIMNUM, DIRS, COHS, TIME, NumTr, recompute)
% Simulate NumTr trials of random population responses for each coh in COH,
% and calculate the principle components of the responses.

[h] = dirnames;
fn  = ['getML_RLFig6SimResponses_' Monk '_' num2str(ELN)  '_' int2str(SIMNUM) '.mat'];
...
if recompute
    ...
    for i = 1:length(DIRS)
        for j = 1:length(COHS)
            ...
            for k = 1:NumTr
                ...
                a     = k0 + COHS(j)*acoh;       % mean firing of MT cell for this trial
    ...
    % compute pca of responses
    [coeff,score,latent] = princomp(rall);
    
    
    save(savepath, 'rall', 'coeff', 'score', 'latent')
