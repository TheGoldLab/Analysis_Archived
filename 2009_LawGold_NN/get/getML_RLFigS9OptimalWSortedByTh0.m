function [wo_sorted, wod_sorted, wocorr_sorted, wocorrd_sorted, wo, wocorr, ind] = getML_RLFigS9OptimalWSortedByTh0(Monk, NSEN, NTUNE, ELN, DIRS, SIMNUM, THBINS, NORMSCALE, TAG, recompute)
% Get optimal weights, sorted by thresholds
% the optimal weights is computed as the normalized fisher linear discriminant,
% without taking into account of the correlations
%
%%
[h] = dirnames;
fn  = ['getML_RLFigS9OptimalWSortedByTh0_' TAG '_Monk' '.mat'];
...
if recompute
    ...
    for nn = 1:length(SIMNUM)
        ...
        fname = ['/getML_RL' TAG '_' Monk '_' int2str(NSEN) '_' int2str(NTUNE) '_' num2str(ELN) '_v' num2str(SIMNUM(nn)) '.mat'];
        load([tdir fname])
        fprintf([fname '\n']);
    ...
    for nn = 1:NTUNE
        ...
        for j = 1:length(THBINS)
            ...
            wo_sorted(j,nn)      = nanmean(wo_(L));
            wod_sorted(j,nn)     = nanse(wo_(L));
            wocorr_sorted(j,nn)  = nanmean(woc_(L));
            wocorrd_sorted(j,nn) = nanse(woc_(L));
        end
    end
