function [p] = getML_RLPerformanceDirCoh(Monk, NSEN, NTUNE, DIRS, COHS, w, MTindex, N, TAG, RECOMPUTE)
...
savepath = [tdir '/getML_RLPerformanceDirCoh_' Monk '_'  TAG '.mat'];
    ...
    for i = 1:size(DIRS,2)
        for j = 1:length(COHS)
            ...
            for nn = 1:N
                ...
                % decision
                choice  = pool>0;
                rew(nn) = choice==(sign(cos(pi/180.*dir_))+1)/2;
            end
            p(i,j) = nansum(rew)/N;
        end
    end
