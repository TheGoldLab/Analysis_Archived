function [FIRA, W, MTindex] = getML_RLSimPerceptualLearningOja(Monk, NSEN, NTUNE, eln, DIRS, MTDIRS, SIMNUM, RECOMPUTE)
...
% 
% % add possion noise
% % with weight normalization (Oja like)
% % not fixed directions

[hdir, ldir, cdir, tdir] = dirnames;
savepath = [tdir '/getML_RLOja_' Monk '_' int2str(NSEN) '_' int2str(NTUNE) '_' num2str(eln) '_v' num2str(SIMNUM) '.mat'];
...
%     w       = sqrt(0.02)*w./wss;
    ...
    for i = 1:N
        ...
        pool(i)   = pool(i)+normrnd(0,sqrt(2*abs(pool(i))));
        ...
        dw  = eln*rew*((choice-p)*r - aconst*(choice-p)^2*w) + lambda*eln*(1-rew)*((1-choice-p)*r - aconst*(1-choice-p)^2*w);
        w   = w + dw;
