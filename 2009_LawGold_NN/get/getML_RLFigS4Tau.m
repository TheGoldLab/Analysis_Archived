function [bt, btd, bl, bld] = getML_RLFigS4Tau(Monk, NSEN, NTUNE, ELN, DIRS, SIMNUM, beta, recompute)
% Fit an exponential function to lapse and thresholds data for simulations
% with different shape parameters of the logistic function

%%
[h] = dirnames;
fn  = ['getML_RLFigS4Tau_' Monk '_' num2str(length(beta)) '.mat'];
...
if recompute
    ...
    bl  = nans(3,length(beta));
    bt  = nans(3,length(beta));
    bld = nans(3,2,length(beta));
    btd = nans(3,2,length(beta));;
    
    
    for bb1 = 1:length(beta)
                coh = [];
                dir = [];
                vt  = [];
                crt = [];
                ...
                for i = 1:length(SIMNUM)
                    ...
                    fname = ['/getML_RLLog_' Monk '_' int2str(NSEN) '_' int2str(NTUNE) '_' num2str(ELN) '_' num2str(beta(bb1)) '_v' num2str(SIMNUM) '.mat'];
                    load([tdir fname])
                    fprintf([fname '\n']);
               ...
               % get lapse time constants
                ...
                L      = coh>0.9 & vt>=0.7;
                x      = [1:N]';
                [bl(:,bb1), bld(:,:,bb1)] = exp_fitd_bino(x(L), 1-crt(L), [], [0 0; 0.5 0.5; 0 N], {100, 68});
                ...
                [bt(:,bb1), btd(:,:,bb1)] = exp_fitWd(tbins(~Lbd), th(~Lbd), thd(~Lbd), ...
                                                                    [], [0.05 0.12; 0.4 1; 0 100000], {100, 68});
