function [performance] = getML_RLFigS2SpecificityOfLearning(Monk, NSEN, NTUNE, ELN, SIMNUM, DIRS, COHS, N, recompute)
% Simulate performance at each COHS at 1s of viewing duration for every
% directions in DIRS

[h] = dirnames;
fn  = ['getML_RLFigS2SpecificityOfLearning_' Monk '_' num2str(ELN)  '_' int2str(SIMNUM) '.mat'];
...
%     fprintf([fname '\n']);
    ...
    performance = nans(length(COHS), length(DIRS)); 
    w           = W(:,:,end);
    for i = 1:length(DIRS)
        for j = 1:length(COHS)
            [i j]
