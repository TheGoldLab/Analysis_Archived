function [FIRA, W, MTindex] = getML_RLSimPerceptualLearningEasyDiff(Monk, NSEN, NTUNE, eln, DIRS, MTDIRS, SIMNUM, DFLAG, RECOMPUTE)
...
% % fixed directions
% % Easy first or difficult first (depending on DFLAG, 0 easy first, 1 difficult first)


[hdir, ldir, cdir, tdir] = dirnames;
savepath = [tdir '/getML_RLEasyDiff_' Monk '_' int2str(DFLAG) '_' int2str(NSEN) '_' int2str(NTUNE) '_' num2str(eln) '_v' num2str(SIMNUM) '.mat'];
...
if RECOMPUTE
    ...
    N    = length(dirsgn);
    if DFLAG
        coh  = linspace(0.016,0.999,N)';
    else
        coh  = linspace(0.999,0.016,N)';
    end
    vt   = 0.7*ones(N,1);
    clear data
    ...
    for i = 1:N
        ...
        % pick random coh
        coh_ = normrnd(coh(i),0.15);
        if coh_<0.016
            coh_=0.016;
        elseif coh_>0.999
            coh_=0.999;
        end
        a     = vt(i)*(k0 + coh_*acoh);       % mean firing of MT cell for this trial
        ...
        FIRA(i,:) = [i, coh_, dir(i), vt(i), pool(i), p, beta, choice, rew];
    end
