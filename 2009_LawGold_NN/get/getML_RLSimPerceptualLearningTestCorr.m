function [FIRA, W, MTindex] = getML_RLSimPerceptualLearningTestCorr(Monk, NSEN, NTUNE, eln, DIRS, SIMNUM, rmax, bdir, bsen, RECOMPUTE)
...
% 
% Input includes parameters defining the correlation structure, used to test how the correlation 
% structure affects the time course and asymptotic values of learning

[hdir, ldir, cdir, tdir] = dirnames;
savepath = [tdir '/getML_RLCorr_' Monk '_' int2str(NSEN) '_' int2str(NTUNE) '_' num2str(eln) '_' num2str(rmax) '_' num2str(bdir) '_' num2str(bsen) '_v' num2str(SIMNUM) '.mat'];
...
if RECOMPUTE
    ...
    rsen  = rmax;       % max correlation between neurons with different sensitivity
    rslp  = -rsen/bsen;  % number of neighbouring neurons that it is correlated to 
    R0_   = [];
    th    = th(:);
    for i = 1:NSEN
...
%     w       = sqrt(0.16)*w./wss;
    ...
    % save data
    save(savepath, 'FIRA', 'W', 'MTindex', 'NSEN', 'NTUNE', 'eln', 'DIRS');
