function [FIRA, W, MTindex] = getML_RLSimPerceptualLearningGenRL00(Monk, NSEN, NTUNE, eln, DIRS, MTDIRS, SIMNUM, RECOMPUTE)
...
savepath = [tdir '/getML_RLGenRL00_' Monk '_' int2str(NSEN) '_' int2str(NTUNE) '_' num2str(eln) '_v' num2str(SIMNUM) '.mat'];
...
if RECOMPUTE
    ...
    for i = 1:N
        ...
        dw  = eln*rew*(2*choice-1)*(r) + lambda*eln*(1-rew)*(-(2*choice-1))*(r);
        w   = w + dw;
        wss = sqrt(sum(sum(w.^2)));
        w   = sqrt(0.02)*w./wss;
