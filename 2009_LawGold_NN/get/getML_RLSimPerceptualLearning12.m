function [FIRA, W, MTindex] = getML_RLSimPerceptualLearning12(Monk, NSEN, NTUNE, eln, DIRS, MTDIRS, SIMNUM, RECOMPUTE)
...
if RECOMPUTE
    ...
    for i = 1:N
        ...
        if choice % rightward 
            dw  = eln*(2*choice-1)*(rew-p)*r;
        else      % leftward
            dw  = eln*(2*choice-1)*(rew-(1-p))*r;
        end
        w   = w + dw;
        wss = sqrt(sum(sum(w.^2)));
        w   = sqrt(0.02)*w./wss;
