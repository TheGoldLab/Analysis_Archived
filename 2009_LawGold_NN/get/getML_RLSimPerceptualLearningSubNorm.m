function [FIRA, W, MTindex] = getML_RLSimPerceptualLearningSubNorm(Monk, NSEN, NTUNE, eln, DIRS, MTDIRS, SIMNUM, RECOMPUTE)
...
% % subtractive weight normalization
% % not fixed directions

[hdir, ldir, cdir, tdir] = dirnames;
savepath = [tdir '/getML_RLSubNorm_' Monk '_' int2str(NSEN) '_' int2str(NTUNE) '_' num2str(eln) '_v' num2str(SIMNUM) '.mat'];
...
if RECOMPUTE
    ...
    for i = 1:N
        ...
        w   = w + dw - sum(sum(dw))/size(dw,1)/size(dw,2);  % subtractive normalization
