function [FIRA, w, MTindex] = getML_RLSimPerceptualLearningTestBound(Monk, NSEN, NTUNE, eln, DIRS, SIMNUM, RECOMPUTE)
% Simulation learning of the motion discrimination task with a
% increasing bound height instead of delta learning
% 

[hdir, ldir, cdir, tdir] = dirnames;
savepath = [tdir '/getML_RLBound_' Monk '_' int2str(NSEN) '_' int2str(NTUNE) '_' num2str(eln) '_v' num2str(SIMNUM) '.mat'];
...
if RECOMPUTE
    ...
    atune = WrappedGaussian(kdir, 1, 40.^2, 180);
    ...
    atune = WrappedGaussian(kdir, 1, 40.^2, 0);
    atune = atune/max(max(atune));          % tuning modulation function
    acoh  = (kps-kns).*atune+kns;           % acoh will be a gaussian function, with peak at pref-slope, and baseline equal to null-slope (which can be a negative number)
    ...
    for i = 1:N
        ...
        FIRA(i,:) = [i, coh(i), dir(i), vt(i), pool(i), nan, nan, choice, rew];
    end
