function [FIRA, W, MTindex] = getML_RLSimPerceptualLearningFine(Monk, NSEN, NTUNE, eln, DIRS, MTDIRS, SIMNUM, RECOMPUTE)
...
if RECOMPUTE
    ...
    for i = 1:N
        ...
        if i<=length(dirsgn)
            atune = WrappedGaussian(kdir, 1, 40.^2, dirsgn(i)*DIRS(2));
            ...
            dirsgn(i) = 2*unidrnd(2)-3;
            COHSS     = [0 3.2 6.4 12.8 25.6 51.2 99.9]/100;
            coh(i)    = COHSS(unidrnd(7));
            vt(i)     = 0.1+exprnd(0.5);
            atune = WrappedGaussian(kdir, 1, 40.^2, dirsgn(i)*DIRS(2));
            atune = atune/max(max(atune));          % tuning modulation function
            acoh  = (kps-kns).*atune+kns;           % acoh will be a gaussian function, with peak at pref-slope, and baseline equal to null-slope (which can be a negative number)
        ...
        FIRA(i,:) = [i, coh(i), dirsgn(i)*DIRS(2), vt(i), pool(i), p, beta, choice, rew];
    end
