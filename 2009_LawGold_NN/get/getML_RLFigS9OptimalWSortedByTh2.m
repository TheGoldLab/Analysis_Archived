function [wo_sorted, wod_sorted, wocorr_sorted, wocorrd_sorted, wo, wocorr, ind] = getML_RLFigS9OptimalWSortedByTh(Monk, NSEN, NTUNE, ELN, DIRS, SIMNUM, THBINS, NORMSCALE, TAG, recompute)
...
%%
[h] = dirnames;
fn  = ['getML_RLFigS9OptimalWSortedByTh2_' TAG '_' Monk '.mat'];
...
if recompute
    ...
    for nn = 1:length(SIMNUM)
        ...
        rsen  = 0.15;       % max correlation between neurons with different sensitivity
        R0_   = [];
        for i = 1:NSEN
            R0_ = [R0_, rsen*ones(NSEN,1)];
        end
        R0_(R0_<0) = 0;
