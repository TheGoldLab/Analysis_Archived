function [FIRA, W, MTindex] = getML_RLSimPerceptualLearningFitSig(Monk, NSEN, NTUNE, eln, DIRS, MTDIRS, SIMNUM, RECOMPUTE)
...
% % Fit sigmoidal function instead of using sequential update 

[hdir, ldir, cdir, tdir] = dirnames;
savepath = [tdir '/getML_RLFitSig_' Monk '_' int2str(NSEN) '_' int2str(NTUNE) '_' num2str(eln) '_v' num2str(SIMNUM) '.mat'];
...
if RECOMPUTE
    ...
    for i = 1:N
        ...
        if i<NBHIST
            beta = 1;
        elseif ~mod(i,NBHIST)
            fits = logist_fit([pool(i-NBHIST+1:i) (sign(dirsgn(i-NBHIST+1:i))+1)/2], 'n');
            beta = fits(1);
        else
            beta = FIRA(i-1,7);
        end
