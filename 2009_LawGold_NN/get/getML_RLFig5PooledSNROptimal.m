function [mpool, sdpool] = getML_RLFig5PooledSNROptimal(Monk, NSEN, NTUNE, ELN, SIMNUM, COH, TIME, recompute)
% compute SNR of pooled responses 
[h] = dirnames;
fn  = ['getML_RLFig5PooledSNROptimal_' Monk '_' int2str(100*COH)  '_' int2str(1000*TIME) '.mat'];
...
if recompute
    ...
    NORMSCALE = sqrt(0.02);
    [a, b, c, d, Wo] = getML_RLFig3OptimalWSortedByTh(Monk, NSEN, NTUNE, ELN, [180 0], SIMNUM, THBINS, NORMSCALE, 0);
    ...
    for i = 1:length(SIMNUM)
        ...
        DIR       = 0;
        atune     = WrappedGaussian(kdir, 1, 40.^2, DIR);
