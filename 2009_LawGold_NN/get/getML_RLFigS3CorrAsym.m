function [performance] = getML_RLFigS3CorrAsym(Monk, DIRS, COHS, rmax, bdir, bsen, N, recompute)
% Simulate performance at each COHS at 1s of viewing duration for every
% directions in DIRS using optimal weights
%
% test how the asymptotic performance is affected by correlations

[h] = dirnames;
fn  = ['getML_RLFigS3CorrAsym_' Monk '_' 'rmax', int2str(length(rmax))  ...
...
if recompute
    ...
    %% sim
    performance = nans(length(COHS), length(rmax), length(bdir), length(bsen)); 
        ...
        for rr2 = 1:length(bdir)
            for rr3 = 1:length(rmax)
                ...
                for j = 1:length(COHS)
                    ...
                    performance(j,rr3,rr2,rr1) = sum(choice)/N;
                end
            end
        end
    end
