function [wo_sorted, wod_sorted, wo, wocorr, ind] = getML_RLFigS10OptimalWSortedByTh(Monk, NSEN, NTUNE, ELN, DIRS, SIMNUM, THBINS, NORMSCALE, WIDTH, recompute)
...
% as a function of tuning width
%%
[h] = dirnames;
fn  = ['getML_RLFigS10OptimalWSortedByTh_' Monk '.mat'];
...
if recompute
    ...
    wo     = nans(NSEN, NTUNE, length(SIMNUM), length(WIDTH));
    ind    = nans(NSEN, NTUNE, length(SIMNUM), length(WIDTH));
    for nn=1:length(WIDTH)
        WIDTH(nn)
        for i = 1:length(SIMNUM)
            ...
            atune = WrappedGaussian(kdir, 1, WIDTH(nn).^2, DIRS(1));
            ...
            atune = WrappedGaussian(kdir, 1, WIDTH(nn).^2, DIRS(2));
            atune = atune/max(max(atune));          % tuning modulation function
            acoh  = (kps-kns).*atune+kns;           % acoh will be a gaussian function, with peak at pref-slope, and baseline equal to null-slope (which can be a negative number)
            ...
            wo(:,:,i,nn)     = reshape(w_,NSEN,NTUNE);

            ind(:,:,i,nn)    = repmat(MTindex',1,NTUNE);
        end
    end
    th = TH(ind);
    ...
    wo_sorted       = nans(length(THBINS), NTUNE, length(WIDTH));
    wod_sorted      = nans(length(THBINS), NTUNE, length(WIDTH));
    ...
    for nn = 1:length(WIDTH)
        for i = 1:NTUNE
            ...
            for j = 1:length(THBINS)
                ...
                wo_sorted(j,i,nn)      = nanmean(wo_(L));
                wod_sorted(j,i,nn)     = nanse(wo_(L));
            end
        end
    end
    save(savepath, 'wo_sorted', 'wod_sorted', 'wo',  'ind')
