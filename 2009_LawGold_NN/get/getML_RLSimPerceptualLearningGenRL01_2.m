function [FIRA, W, MTindex] = getML_RLSimPerceptualLearningGenRL01(Monk, NSEN, NTUNE, eln, DIRS, MTDIRS, SIMNUM, RECOMPUTE)
...
% % Generalized RL rule, post-synaptic gating

[hdir, ldir, cdir, tdir] = dirnames;
savepath = [tdir '/getML_RLGenRL01_' Monk '_' int2str(NSEN) '_' int2str(NTUNE) '_' num2str(eln) '_v' num2str(SIMNUM) '.mat'];
...
if RECOMPUTE
    ...
    for i = 1:N
        ...
        dw  = eln*rew*(2*choice-1)*(r-mean(mean(r))) + lambda*eln*(1-rew)*(1-(2*choice-1))*(r-mean(mean(r)));
        w   = w + dw;
        w   = sqrt(0.02)*w./wss;
