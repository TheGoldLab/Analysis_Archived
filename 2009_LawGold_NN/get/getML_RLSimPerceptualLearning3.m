function [FIRA, W, MTindex] = getML_RLSimPerceptualLearning(Monk, NSEN, NTUNE, eln, DIRS, MTDIRS, SIMNUM, RECOMPUTE)
...
if RECOMPUTE
    ...
    LMT   = ismember(kdir, MTDIRS);  % store MT responses to compute choice probability
...
%     R0_    = rthmin*rthmin';
    ...
    %   Define constants
    %%%
    FIRA    = nans(N,9+sum(sum(LMT)));          % [trial #, coh, dir, vt, pooled response, reward prob, ...
    ...
    for i = 1:10000
        ...
        %   store data
        %%%
        MTr       = r(LMT)';    % [MT responses from cell MTind(1) to MTind(end) for neurons tuned to -ve dir,
        
        %  MT responses from cell MTind(1) to MTind(end) for neurons tuned to +ve dir]
        FIRA(i,:) = [i, coh(i), dir(i), vt(i), pool(i), p, beta, choice, rew, MTr];
    end
