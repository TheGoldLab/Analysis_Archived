function [mpool, sdpool] = getML_RLFig5PooledSNR(Monk, NSEN, NTUNE, ELN, SIMNUM, COH, TIME, recompute)
% compute SNR of pooled responses 
[h] = dirnames;
fn  = ['getML_RLFig5PooledSNR_' Monk '_' int2str(100*COH)  '_' int2str(1000*TIME) '.mat'];
...
if recompute
    ...
    mpool  = nans(size(W,3),length(SIMNUM));
    sdpool = nans(size(W,3),length(SIMNUM));
