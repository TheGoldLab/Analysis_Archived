function vals_ = ddOU4(fits, data)
% function vals_ = ddOU4(fits, data)
%
% 3 parameters: A, alpha, lapse, time lag
%
...
%       fits(3) ... lambda ("lapse")
%       fits(4) ... time lag (time when time-dependent performance begin)
%       
...
%      lapse        0.0001    0.18];
    ...
    acm   = fits(1).*data(:,1).^M;
    mu    = acm./fits(2).*(exp(fits(2).*(data(:,2)-fits(4)))-1);
