function vals_ = quickTsFixLapse(fits, data, spec, lp)
% function vals_ = quickTsFixLapse(fits, data, spec, lp)
% 
...
% P = G + (1 - G - L)P*
%
% Note: If spec for lapse is 0, set lapse rate to lp. If lp is empty, set
%       lapse to 0.

% 5/30/07 modified from quickTs.m by jcl 
...
if nargin < 2 || isempty(fits)
    ...
    if spec(3)
        ...
        % fix lapse rate to lp if lp is smaller than 0.18, else set lapse
        % as 0.18
