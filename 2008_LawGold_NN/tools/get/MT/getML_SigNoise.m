function [sig fano SNR] = getML_SigNoise(fname, crtf, nvf, dirf, recompute)
...
if recompute
    ...
    for i = 1:length(fn)
        if usable(i) == 1;
            ...
            % get rate
            for j = 1:14
                % get selection array for viewing time
                if strcmp(dtype, 'MT')
                    bt = getFIRA_ecodeTimesByName('dot_on', 0);
                    et = getFIRA_ecodeTimesByName('dot_off', 0);
                    Lt = logical(ones(size(L(:,j))));
                else
                    bt = getFIRA_ecodeTimesByName('dot_on', 400);
                    et = getFIRA_ecodeTimesByName('dot_on', 500);
                    Lt = (getFIRA_ecodeTimesByName('dot_off', 0)-getFIRA_ecodeTimesByName('dot_on', 0))>=500;
                end    
                r = getFIRA_rate(L(:,j)&Lt, getFIRA_spikeByID(uid(i)), ... 
...
%             cla
%             hold on
%             plot([0 3.2 6.4 12.8 25.6 51.2 99.9], rM(1:7), 'k.')
%             plot([0 3.2 6.4 12.8 25.6 51.2 99.9], rM(8:14), 'r.')
%             hold off
%             
%             subplot(1,2,2)
%             cla
%             hold on
%             plot([0 3.2 6.4 12.8 25.6 51.2 99.9], rSD(1:7).^2, 'k.')
%             plot([0 3.2 6.4 12.8 25.6 51.2 99.9], rSD(8:14).^2, 'r.')
%             hold off
%             pause
